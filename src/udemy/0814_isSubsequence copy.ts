...
export function isSubsequence(s: string, t: string): boolean {
  ...
  const findNextIndex = (positions: number[], target: number): number => {
    ...
    while (left <= right) {
      ...
      let currentNum = positions[mid]!;

      if (currentNum < target) {
        left = mid + 1;
      } else if (currentNum > target) {
        result = currentNum; // potential result
        right = mid - 1;
      }
    }
  ...
  for (let ch of s) {
    // check if stored doesn't have ch --> return false.

    if (!stored[ch]) return false;

    // use binary search to find the next index in the s
    //  the next index should be bigger than previous index
    // if not, it will return -1

    let nextIndex = findNextIndex(stored[ch], preIndex);

    if (nextIndex === -1) return false;

    // set previous index equal to nextIndex
