...
export function maxSubarraySum(arr: number[], num: number): number | null {
  ...
  // sliding
  /**
   * 1. take the first space sum based on param num
   * 2. slide from the left to the right based on param num
   */
  /**
   * 2 sums:
   * maxSum
   ...
   * ---
   * need to create a loop to count for the first sum -> from the index 0 -> num
   * ==> we will have tempSum = maxSum
   *
   * Then we create a sliding based on num
   * --> create a loop --> start = num ==> slide from left t right
   * each slide time  ==> we will remove the last on the left
   * and plus new element with index i
   * => tempSum
